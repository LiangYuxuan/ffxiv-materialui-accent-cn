...
namespace MaterialUI {
	...
	public class Updater {
        ...
        public const string repoAccentSHA = "7874dbfe0e174c91a19d0f10c3fd5e9225797177";
		...
		private Dir PopulateDir(Repo repo, string repoName, string repoSHA = "master") {
			...
			foreach(RepoFile file in repo.tree) {
				...
				if(file.type == "tree") {
					...
					curdir.files[path[path.Length - 1]] = (file.sha, String.Format("https://ghproxy.com/https://raw.githubusercontent.com/{0}/{1}/{2}", repoName, repoSHA, file.path));
				}
			}
		...
		public void Update() {
			Task.Run(async() => {
				...
				string resp = await httpClient.GetStringAsync(String.Format("https://ghproxy.com/https://raw.githubusercontent.com/LiangYuxuan/ffxiv-materialui-accent-cn/tree/{0}/{1}", repoMaster, repoMasterSHA));
